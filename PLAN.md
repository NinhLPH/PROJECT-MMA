...
* `GET /api/trainers/:id/schedules` : Truy vấn danh sách các khung giờ tập trống (`isBooked = false`) của một PT cụ thể theo ngày. Thêm query `includeBooked=true` khi App cần hiển thị toàn bộ ca, trong đó các ca đã đặt (`isBooked = true`) được làm mờ và disabled.
